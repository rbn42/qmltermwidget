...
{
  QRect cursorRect = imageToWidget( QRect(cursorPosition(),QSize(2,1)) ); 
  update(cursorRect);
}
...
void TerminalDisplay::copyScreenClipboard()
{
  if ( !_screenWindow )
      return;

  QString text = _screenWindow->screenText(_preserveLineBreaks);
  if (!text.isEmpty())
    QApplication::clipboard()->setText(text);
}
